...
DS = CumS - C0
D1245 = C1245-C0
SD1245A= ' (%5.2f)' % D1245.weighted(weights).mean().values
print('SD1245A',SD1245A)

print('min',D1.min().values)
drmin = np.min([D1.min(), D2.min(), D4.min(), D5.min(), DS.min(), D1245.min()])
drmax = np.max([D1.min(), D2.max(), D4.max(), D5.max(), DS.max(), D1245.max()])
...
if True:
    xr_llhplot2(D1, ax=axf[0],clevs=dlevs,cmap=dmap,title=pref1+sC1AD, ylabels=False,cbar=None)
...
sC2AD = ' (%5.2f)' % C2AD
if True:
    xr_llhplot2(D2, ax=axf[1],clevs=dlevs,cmap=dmap,title=pref2+sC2AD, ylabels=False,cbar=None)
    #pltllbox2([-150.,-110.],[0.,30.],ax=axes[1])
    pltllbox2([-110.,-70.],[-30.,0.],ax=axf[1])
    #pltllbox2([-25.,15.],[-30.,0.],ax=axes[1])

C4A = C4.weighted(weights).mean()
C4AD = C4A - C0A
#print(C3AD)
sC4AD = ' (%5.2f)' % C4AD
if True:
    xr_llhplot2(D4, ax=axf[2],clevs=dlevs,cmap=dmap,title=pref4+sC4AD, ylabels=False,cbar=None)
    #pltllbox2([-150.,-110.],[0.,30.],ax=axes[2])
    #pltllbox2([-110.,-70.],[-30.,0.],ax=axes[2])
    #pltllbox2([-25.,15.],[-30.,0.],ax=axf[2])
    
C5A = C5.weighted(weights).mean()
C5AD = C5A - C0A
#print(C3AD)
sC5AD = ' (%5.2f)' % C5AD
if True:
    xr_llhplot2(D5, ax=axf[3],clevs=dlevs,cmap=dmap,title=pref5+sC5AD, ylabels=False,cbar=None)
    #pltllbox2([-150.,-110.],[0.,30.],ax=axes[2])
    #pltllbox2([-110.,-70.],[-30.,0.],ax=axes[2])
    pltllbox2([-25.,15.],[-30.,0.],ax=axf[3])

sCSAD = ' (%5.2f)' % CSAD
xr_llhplot2(DS, ax=axf[4],clevs=dlevs,cmap=dmap,title=prefS+sCSAD, ylabels=False,cbar=None)
#pltllbox2([-150.,-110.],[0.,30.],ax=axf[4])
#pltllbox2([-110.,-70.],[-30.,0.],ax=axf[4])
#pltllbox2([-25.,15.],[-30.,0.],ax=axf[4])

sCSAD = 'xx'#' (%5.2f)' % CSAD
xr_llhplot2(D1245, ax=axf[5],clevs=dlevs,cmap=dmap,title=pref1245+SD1245A, ylabels=False)
...
xr_cshplot(DV, lon_e3sm, lat_e3sm, clevs=dlevs, ax=axf[4],ylabels=False)
